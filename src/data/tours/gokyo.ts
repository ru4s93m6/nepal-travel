...
export const gokyoTour: TourData = {
  ...
  locales: {
    ...
    'zh-tw': {
      ...
      overview: '高喬湖（Gokyo Lake）健行是傳統聖母峰基地營（EBC）路線的最佳替代方案。這條路線避開了最擁擠的主幹道，帶領您穿越寧靜的雪巴村落，最終抵達被譽為「人間仙境」的高喬湖泊群。清澈湛藍的冰川湖泊與世界最大的 Ngozumpa 冰河交相輝映。旅程的最大亮點是登頂海拔 5,357 公尺的高喬里（Gokyo Ri），在這裡您可以同時將包含聖母峰（Everest）、洛子峰（Lhotse）、馬卡魯峰（Makalu）與卓奧友峰（Cho Oyu）在內的四座 8,000 公尺級巨峰盡收眼底。\n\n請注意：本 13 天行程為為預估標準，我們會根據您的體能狀況與高山適應程度彈性調整步伐，確保您能在安全的前提下完成這趟的旅程。',
      highlights: [
